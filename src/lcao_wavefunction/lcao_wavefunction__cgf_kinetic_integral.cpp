...
//
    ...
    basis_set a_set(type(atom_a), level);
    basis_set b_set(type(atom_b), level);
    unsigned int m_primitive = tools::first_gf(level, a_orbital); 
    unsigned int n_primitive = tools::first_gf(level, b_orbital);
//
    for(unsigned int i = 1; i <= a_set.basis_set::size(); ++i)
    {
        for(unsigned int j = 1; j <= b_set.basis_set::size(); ++j)
        {
            ...
            #pragma omp parallel sections num_threads(9)
            {
                #pragma omp section
                {
                    a = a_set.basis_set::function_coeff(m_primitive)
                      * b_set.basis_set::function_coeff(n_primitive);
                }
                #pragma omp section
                 {
                    b = gf_norm(a_set.basis_set::function_expon(m_primitive),
                                              angular_component_x(a_orbital),
                                              angular_component_y(a_orbital),
                                              angular_component_z(a_orbital))
                      * gf_norm(b_set.basis_set::function_expon(n_primitive),
                ...
                #pragma omp section
                {
                    c = kinetic_integral(x,
                                         ...
                                         angular_component_x(a_orbital),
                                         angular_component_x(b_orbital),
                                         a_set.basis_set::function_expon(m_primitive),
                                         b_set.basis_set::function_expon(n_primitive));
                }
                #pragma omp section
                {
                ...
                #pragma omp section
                {
                    i = gf_overlap_factor(a_set.basis_set::function_expon(m_primitive),
                                          b_set.basis_set::function_expon(n_primitive),
